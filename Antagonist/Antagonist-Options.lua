...
function Antagonist:RegisterOpts()
	local guiOpts = {
		...
		args = {
			...
			stop = {name = L["Stop"], desc=L["DescStop"], type = "execute", func = function() self:StopEverything() end, order = 14},
		},
	}
	
	self:RegisterChatCommand({"/antcl","/antagonistcl"}, guiOpts)
	self:RegisterChatCommand({ "/ant", "/antagonist"}, function() waterfall:Open('Antagonist') end)
	waterfall:Register('Antagonist', 'aceOptions', guiOpts, 'title','Antagonist Revision: 15751','colorR', 0.498039, 'colorG', 1, 'colorB', 0.831373) 
